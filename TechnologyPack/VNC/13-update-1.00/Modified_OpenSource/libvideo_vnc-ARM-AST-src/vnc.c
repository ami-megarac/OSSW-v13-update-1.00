...
int startCapture()
{
	...
	{
		// On first launch resolution values from sourcemode x,y are wrong. Fix it by referring to g_tile_info x,y values
	    	frame_hdr_vnc.res_x = g_tile_info.width; //((frame_hdr_t *)p_frame_hdr)->SourceMode_X;
		frame_hdr_vnc.res_y = g_tile_info.height;//((frame_hdr_t *)p_frame_hdr)->SourceMode_Y;
